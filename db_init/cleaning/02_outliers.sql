...
SELECT 
    ...
    -- 1. Rooms Logic: Case-insensitive checks
    CASE 
    ...
    -- 2. Bathrooms Logic: Case-insensitive checks
    CASE 
        WHEN LOWER(type_bien) = 'terrain' THEN NULL
    ...
    -- 3. Floor Logic: Case-insensitive checks
    CASE 
    ...
    -- 4. Construction Year Logic: Case-insensitive checks
    CASE 
        WHEN LOWER(type_bien) = 'terrain' THEN NULL
...
WHERE 
    ...
    AND (
        (LOWER(transaction) = 'location' AND CAST(prix_raw AS NUMERIC) BETWEEN 400 AND 60000) OR
        (LOWER(transaction) = 'vente' AND CAST(prix_raw AS NUMERIC) BETWEEN 80000 AND 15000000) OR
        (transaction IS NULL) 
    );